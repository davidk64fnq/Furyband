	...
	["hooks"]       =
	{
		...
		[HOOK_READ] = function(obj)
			local f1, f2, f3, f4, f5, esp = object_flags(obj)
            	...
            	if band(f3,TR3_IGNORE_FIRE) then return FALSE end
	...
	["hooks"]       =
	{
		...
		[HOOK_QUAFF] = function(obj)
			if magik(9) == TRUE then
				msg_print("Your breath spoils the potion!")
				return TRUE, FALSE
			else
				return FALSE
			end
...
{
	["color"]       = TERM_L_RED,
	["name"]	= "Dark Maia Realm",
	["get_text"]    = "You feel more attuned to the Dark Maia realm.",
	["lose_text"]   = "You lose your attunement to the Dark Maia realm.",
	["desc"]	=
	{
			  "  Provides access to the Maiaology school skill and the use of Maiaic equipment",
			  "  You need Dark Maia Spirit, Dark Maia Hide and Dark Maia Breath to activate it"
	},
	["depends"]     =
	{
...
CORRUPT_TROLL_BLOOD = add_corruption
{
	...
	["desc"]	=
	{
			  ...
			  "  But it will make your presence more noticeable",
	},
	["can_gain"] =	function()
