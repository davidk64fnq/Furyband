	...
	if(c_ptr.m_idx > 0) then
		...
		if(m_ptr.status >= MSTATUS_NEUTRAL) then
			...
			if (cave_floor_bold(y, x) == TRUE) then
				...
				if (cave_is(cave(player.py, player.px), FF1_FLOOR) == TRUE) or (m_ptr.flags2 == RF2_PASS_WALL) or ((cave(player.py,player.px).feat == 16) and (m_ptr.d_char == 'S')) then
                    ...
                    ss = monster_desc(m_ptr, 0)
                    ss = strupper(strsub(ss,1,1)) .. (strsub(ss,0 - (strlen(ss) - 1)))
					msg_print(format("%s is in your way!", ss))
